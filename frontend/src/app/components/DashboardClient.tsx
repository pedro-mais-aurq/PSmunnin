...
import { api, isRetryableRequestError } from "@/lib/api";
import type {
  Lead,
  Search,
  SearchDetail,
...
const POLLING_RETRY_DELAYS_MS = [
  3_000,
  6_000,
  12_000,
];

const POLLING_RETRY_MAX_DELAY_MS =
  15_000;

function getPollingRetryDelayMs(
  retryCount: number,
): number {
  if (
    retryCount
    <= POLLING_RETRY_DELAYS_MS.length
  ) {
    return POLLING_RETRY_DELAYS_MS[
      retryCount - 1
    ];
  }

  return POLLING_RETRY_MAX_DELAY_MS;
}
